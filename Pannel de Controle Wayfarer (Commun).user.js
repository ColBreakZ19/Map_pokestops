...
;function wrapper(plugin_info) { // eslint-disable-line no-extra-semi
	...
	const mapLayers = {
		Potentel: {
			color: 'grey',
			title: 'Potentiels',
			optionTitle: 'Potentiel'
		},
		Attente: {
			color: 'yellow',
			title: 'En attente',
			optionTitle: 'En attente'
		},
		Soumis: {
			color: 'orange',
			title: 'Soumit',
		...
		Potentieledit: {
			...
			title: 'Potentiel edit',
			optionTitle: 'Edit location. Potentiel'
		},
		Evoieedit: {
			color: 'purple',
			title: 'Edit envoyé',
			optionTitle: 'Edit location. Envoyé'
        },
		EditApprouve: {
			color: 'green',
			title: 'Edit approuvé',
			optionTitle: 'Edit location. Approuvé !'
		}
	};
	...
	function drawInputPopop(latlng, markerData) {
		...
		if (id !== '') {
			formContent += '<a style="padding:4px; display: inline-block;" id="deletePortalCandidate"></a>';
		}
	...
	function showDialog() {
		...
		const html =
			`<p><label for="txtScriptUrl">Url du script</label><br><input type="url" id="txtScriptUrl" spellcheck="false" placeholder="https://script.google.com/macros/***/exec"></p>
			 <p><a class='wayfarer-refresh'>Actualiser</a></p>
			 <p><input type="checkbox" id="chkShowTitles"><label for="chkShowTitles">Afficher les titres</label></p>
			 <p><input type="checkbox" id="chkShowRadius"><label for="chkShowRadius">Zone de submission</label></p>
			 <p><input type="checkbox" id="chkShowInteractRadius"><label for="chkShowInteractRadius">Zone d'interaction</label></p>
			`;

		const container = dialog({
			width: 'auto',
			html: html,
			title: 'Panel de controle Wayfarer',
			buttons: {
				OK: function () {
